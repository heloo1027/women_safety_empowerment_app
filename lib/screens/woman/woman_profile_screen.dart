...
Future<void> _pickAndUploadResume() async {
  try {
    ...
    // Cloudinary upload
    String cloudName = 'dztobyinv';
    String uploadPreset = 'resume_upload_preset'; // Create a separate preset for PDFs
    final uri = Uri.parse('https://api.cloudinary.com/v1_1/$cloudName/raw/upload');

    final request = http.MultipartRequest('POST', uri)
      ..fields['upload_preset'] = uploadPreset
      ..files.add(await http.MultipartFile.fromPath('file', _resumeFile!.path, filename: safeFileName));
    ...
    if (response.statusCode == 200) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Resume uploaded successfully: $safeFileName")),
      );
    } else {
      print('Cloudinary upload failed: ${response.statusCode}');
      ScaffoldMessenger.of(context).showSnackBar(
